...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
  <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-4">{$_('settings.syncStatus')}</h2>
    ...
    <div>
      <p class="text-sm flex items-center gap-2">
        {#if isOnline}
          ...
          <span class="text-gray-600 dark:text-gray-400">{$_('common.online')}</span>
        {:else}
          <svg class="w-4 h-4 text-red-600 dark:text-red-400" fill="currentColor" viewBox="0 0 24 24">
            <circle cx="12" cy="12" r="8" />
          </svg>
          <span class="text-gray-600 dark:text-gray-400">{$_('common.offline')}</span>
        {/if}
      </p>
      {#if hasPendingSync}
